...
| Failed-test reports | Failed and flaky tests with every attempt, grouped by stage, job or test run, their bugs and whether one is open, run history, and a compact searchable HTML report with JSON and text attachments | `Get-AdoTestRun`, `Get-AdoBuildTestFailure`, `Export-AdoBuildTestFailure` |
