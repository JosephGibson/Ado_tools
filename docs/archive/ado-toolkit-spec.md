...
| Job or stage re-attempt | Separate test runs in the same build containing the same identity | Nested stage/phase/job attempt counters when exposed, then run `startedDate` and run ID |
...
| V-02 | `isformatted="true"` marks HTML content; unformatted strings are plain text | 2026-09-15, 1B: offline routing, source retention, rich-text, hostile-content, nested-encoding and French fixtures pass. The server's isformatted convention remains unconfirmed; live check required. Audit correction 2026-09-17 (F15): the live check no longer equates markup presence with formatting mode; formatted plain text and literal angle brackets are valid synthetic controls. Server semantics remain open pending visual comparison. [Audit evidence](plans/server-2020-audit-fixes.md). | S1 |
...
| V-06 | WIQL 20,000-result cap and its error shape | Offline 3.1 (2026-09-15): toolkit policy tested; 20,000 or more IDs without `-Top` raise an error with no output, explicit `-Top` sets `LimitApplied`. A synthetic 400 body whose message starts with `VS402337` [V-06] is surfaced with the narrowing hint; other 400s are unchanged. Server truncation or error shape (status, `typeKey`, code) unconfirmed; opt-in live check added. Audit correction 2026-09-17 (F03): encoded/BOM error JSON preserves VS402337 and the narrowing hint. Synthetic UTF-16 French error tested; actual server error shape remains open. [Audit evidence](plans/server-2020-audit-fixes.md). | S3 |
| V-07 | Error body fields (`message`, `typeKey`) and `ActivityId` header | Live check. Audit correction 2026-09-17 (F03): shared JSON decoding honors BOM/charset, retains bounded error bodies and status-based translation for unreadable, HTML or empty bodies. Offline only; live confirmation remains open. [Audit evidence](plans/server-2020-audit-fixes.md). | S0 |
...
| V-10 | `fields` and `$expand` cannot be combined | Offline 1A (2026-09-15): local parameter-set exclusion and both request modes tested. Server combination unconfirmed; raw live probe added. Audit correction 2026-09-17 (F14): accepted fields+expand now contradicts this hypothesis (FAIL); a 400 confirms exclusion only when both individual control requests succeed. Other failures are INCONCLUSIVE. Server exclusion remains open. [Audit evidence](plans/server-2020-audit-fixes.md). | S1 |
